#!/usr/bin/env python3
"""
BioIntelOS SaaS Dashboard Generator
生成完整的 AI Native 生物医学数据平台 UI
"""
from pathlib import Path

OUTPUT = Path(__file__).parent / "biointel-saas-dashboard.html"
...
/* ── Right Sidebar ── */
.right-sidebar{{width:340px;background:#fff;border-left:1px solid #E5E7EB;
  padding:24px 20px;overflow-y:auto}}
.ai-card{{background:#F9FAFB;border-radius:12px;padding:20px;margin-bottom:24px}}
.ai-header{{display:flex;justify-content:space-between;align-items:center;margin-bottom:16px}}
.ai-title{{font-size:16px;font-weight:600;color:#1F2937}}
.ai-avatar{{width:48px;height:48px;border-radius:50%;background:#16C25B;
  display:flex;align-items:center;justify-content:center;margin-bottom:12px}}
.ai-welcome{{font-size:13px;color:#374151;line-height:1.6;margin-bottom:16px}}
.ai-suggestions{{display:flex;flex-direction:column;gap:8px;margin-bottom:16px}}
.ai-suggestion{{padding:10px 14px;background:#fff;border:1px solid #E5E7EB;border-radius:8px;
  font-size:12px;color:#374151;cursor:pointer;transition:all .2s}}
.ai-suggestion:hover{{border-color:#16C25B;color:#16C25B}}
.ai-input-wrap{{position:relative}}
.ai-input{{width:100%;padding:12px 44px 12px 14px;border:1px solid #E5E7EB;
  border-radius:8px;font-size:13px}}
.ai-send{{position:absolute;right:8px;top:50%;transform:translateY(-50%);
  width:32px;height:32px;background:#16C25B;border-radius:50%;border:none;
  color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center}}

.recommend-section{{margin-bottom:24px}}
.recommend-title{{font-size:15px;font-weight:600;color:#1F2937;margin-bottom:12px}}
.recommend-item{{background:#F9FAFB;border-radius:8px;padding:12px;margin-bottom:8px;
  cursor:pointer;transition:all .2s}}
.recommend-item:hover{{background:#EAF8EF}}
.recommend-id{{font-size:12px;font-weight:600;color:#16C25B;margin-bottom:4px}}
.recommend-name{{font-size:12px;color:#374151;line-height:1.4}}
.recommend-time{{font-size:11px;color:#9CA3AF;margin-top:4px}}
</style>
</head>
<body>
